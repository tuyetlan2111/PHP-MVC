...
<header id="topbar" class="ph10">
    <div class="topbar-right hidden-xs hidden-sm mt5 mr35">
        <a href="<?php echo BASE_URL;?>admin/add_book" class="btn btn-primary btn-sm ml10" title="Thêm sản phẩm">
            ...
            <a href="#" class="btn btn-primary btn-sm ml10" title="Sửa sản phẩm">
            <span>Sửa khách hàng</span></a>
            
            <a href="#" class="btn btn-primary btn-sm ml10" title="Xóa sản phẩm">
            ...
            <div class="col-xs-12">
                <div class="panel">
                    <div class="panel-heading">
                       ...
                       <b> <?php print_r(count($ds_employee));?> </b>                  
                        </span>
                    </div>
...
$result='<tbody><tr>';
if(isset($ds_employee)){
    for($i = 0; $i< count($ds_employee);$i++){
        ...
        $result .=  '<td> '.($ds_employee[$i]['user_id']).'</td>';
    $result .= '<td class="">'.($ds_employee[$i]['username']).'</td>';
    $result .= '<td class="">'.($ds_employee[$i]['email']).'</td>';
    $result .= '<td class="">'.($ds_employee[$i]['phone']).'</td>';
    $result .= '<td class="">'.($ds_employee[$i]['address']).'</td>';
    $result .= ' </tr></tbody>';
    }
    echo $result;
}
